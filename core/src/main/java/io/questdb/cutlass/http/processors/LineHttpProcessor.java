...
import io.questdb.std.str.Utf8s;
 ...
 public class LineHttpProcessor implements HttpRequestProcessor, HttpMultipartContentListener {
     ...
     public LineHttpProcessor(CairoEngine engine, int recvBufferSize, int maxResponseContentLength, LineHttpProcessorConfiguration configuration, LineMetrics lineMetrics) {
         this.engine = engine;
     ...
     public void onHeadersReady(HttpConnectionContext context) {
         ...
         if (precision != null) {
             ...
             if ((len == 1 && precision.byteAt(0) == 'n') || (len == 2 && precision.byteAt(0) == 'n' && precision.byteAt(1) == 's')) {
                 ...
                 state.reject(PRECISION_NOT_SUPPORTED, "precision is not supported", context.getFd());
                 return;
             }
         } else {
             timestampPrecision = ENTITY_UNIT_NANO;
